...
typedef vl_codec_handle_t (*fn_init)(vl_codec_id_t codec_id,
                                        ...
                                        qp_param_t* qp_tbl);
typedef encoding_metadata_t (*fn_generate_header)(vl_codec_handle_t codec_handle,
                                                         unsigned char *pHeader,
                                                         unsigned int *pLength);
typedef encoding_metadata_t (*fn_encode)(vl_codec_handle_t handle,
...
{
    VLOG(DEBUG, "amvenc_ports multi loadModule!, load_count:%d\n", encoder_ops.load_count);
    ...
    if (encoder_ops.load_count == 1) {
        ...
        if (encoder_ops.libHandle != NULL) {
            encoder_ops.init = (fn_init)dlsym(encoder_ops.libHandle, "vl_multi_encoder_init");
            if (encoder_ops.init == NULL) {
                VLOG(ERR, "dlsym for vl_multi_encoder_init failed");
                goto exit;
            }

            encoder_ops.generate_header = (fn_generate_header)dlsym(encoder_ops.libHandle, "vl_multi_encoder_generate_header");
            ...
            encoder_ops.encode = (fn_encode)dlsym(encoder_ops.libHandle, "vl_multi_encoder_encode");
            if (encoder_ops.encode == NULL) {
                VLOG(ERR, "dlsym for vl_multi_encoder_encode failed");
                goto exit;
            }

            encoder_ops.destroy = (fn_destroy)dlsym(encoder_ops.libHandle,"vl_multi_encoder_destroy");
...
amvenc_handle_t amvenc_multi_init(amvenc_codec_id_t vcodec_id,
                                       amvenc_info_t vcodec_info,
                                        amvenc_qp_param_t* qp_param)
{
    vl_codec_id_t codec_id = CODEC_ID_NONE;
    vl_encode_info_t encode_info;
    qp_param_t qp_tbl;
    ...
    switch (vcodec_id)
    {
        case AML_CODEC_ID_NONE:
            codec_id = CODEC_ID_NONE;
            break;
        case AML_CODEC_ID_VP8:
            codec_id = CODEC_ID_H261;
            break;
        case AML_CODEC_ID_H261:
            codec_id = CODEC_ID_H261;
            break;
        case AML_CODEC_ID_H263:
                                                              ...
                                                              unsigned int *pLength)
{
    ...
    if (encoder_ops.generate_header)
        encoding_metadata = encoder_ops.generate_header((vl_codec_handle_t)handle, pHeader, pLength);
                                           ...
                                           amvenc_buffer_info_t *ret_buffer_info)
{
    ...
    if (encoder_ops.encode)
        encoding_metadata = encoder_ops.encode((vl_codec_handle_t)handle, enc_frame_type, out, &inbuf_info, &ret_buf);
