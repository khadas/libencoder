...
static int set_input(gx_fast_enc_drv_t* p, ulong *yuv, uint32_t enc_width, uint32_t enc_height, AMVEncBufferType type, AMVEncFrameFmt fmt) {
    ...
    if ((type == PHYSICAL_BUFF) && (pitch % 32 != 0)) {
        if ((src->plane[0] != 0) && (src->mmapsize != 0)) {
            LOGAPI("Release old mmap buffer :0x%lx, size:0x%lx. fd:%d", src->plane[0], src->mmapsize, p->fd);
            munmap((void*) src->plane[0], src->mmapsize);
            src->plane[0] = 0;
            src->mmapsize = 0;
        } else if ((src->plane[0] != 0) || (src->mmapsize != 0)) {
            LOGAPI("Buffer Ptr error:0x%lx, size:0x%lx. fd:%d", src->plane[0], src->mmapsize, p->fd);
        }
...
//    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            p->total_encode_time += total_time;
            LOGAPI("start_intra_cbr: need time: %d us, base qp: %d, frame num:%d, fd:%d", total_time, p->quant, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}
...
static AMVEnc_Status start_ime_one_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        LOGAPI("start_ime_one_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_PICTURE_READY) {
        ...
        if (p->logtime) {
            ...
            total_time = p->end_test.tv_sec - p->start_test.tv_sec;
            total_time = total_time * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            LOGAPI("start_ime_one_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
            p->total_encode_time += total_time;
        }
    }
    return ret;
...
static AMVEnc_Status start_intra_one_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        LOGAPI("start_intra_one_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            p->total_encode_time += total_time;
            LOGAPI("start_intra_one_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}
...
static AMVEnc_Status start_ime_two_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        LOGAPI("start_ime_two_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_PICTURE_READY) {
        ...
        if (p->logtime) {
            ...
            total_time = p->end_test.tv_sec - p->start_test.tv_sec;
            total_time = total_time * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            LOGAPI("start_ime_two_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
            p->total_encode_time += total_time;
        }
    }
    return ret;
...
static AMVEnc_Status start_intra_two_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        LOGAPI("start_intra_two_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            p->total_encode_time += total_time;
            LOGAPI("start_intra_two_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}
...
static int get_block_resolution(gx_fast_enc_drv_t* p, uint32_t mb_width, uint32_t mb_height)
{
    ...
    LOGAPI("EB block width:%d, height:%d mb_width:%d, mb_height:%d, block_width_num:%d, block_height_num:%d, block_width_n:%d, block_height_n:%d, block_width:%d, block_height:%d",
...
void* GxInitFastEncode(int fd, amvenc_initpara_t* init_para) {
    ...
    if ((ret) || (buff_info[0] == 0)) {
        LOGAPI("InitFastEncode -- old venc driver. no buffer information! fd:%d", p->fd);
        free(p);
        return NULL;
    }
