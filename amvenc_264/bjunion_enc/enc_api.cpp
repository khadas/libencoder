...
AMVEnc_Status AMInitRateControlModule(amvenc_hw_t *hw_info)
{
    ...
    if (grc[hw_info->dev_id]->Initialize != NULL) {
        if (!(&(hw_info->init_para))) {
            VLOG(ERR,"AMInitRateControlModule: init_para NULL!");
            return AMVENC_MEMORY_FAIL;
        }
    ...
    if (!hw_info->rc_data) {
        VLOG(ERR,"AMInitRateControlModule Fail, dev type:%d. fd:%d",hw_info->dev_id, hw_info->dev_fd);
        return AMVENC_FAIL;
    }
...
AMVEnc_Status InitAMVEncode(amvenc_hw_t *hw_info, int force_mode)
{
    ...
    VLOG(NONE,"AMVENC_AVC_IOC_GET_DEVINFO: dev_info=%s", dev_info);

    if ((iret<0) || (dev_info[0] == 0)) {
        VLOG(ERR,"The old encoder driver, not support query the dev info. set as M8 type!");
        hw_info->dev_id = M8_FAST;
    }else if ((strcmp(dev_info, (char *)AMVENC_DEVINFO_M8) == 0)
      ||(strcmp(dev_info, (char *)AMVENC_DEVINFO_G9) == 0)){
        VLOG(DEBUG,"shitty m8");
        hw_info->dev_id = M8_FAST;
...
//        }
        ...
        VLOG(DEBUG,"hw.encoder.forcemode = %d, dev_id=%d. fd:%d", value, hw_info->dev_id, fd);
    }
    if ((hw_info->dev_id <= NO_DEFINE) || (hw_info->dev_id >= MAX_DEV)) {
        VLOG(ERR,"Not found available hw encoder device, fd:%d", fd);
        close(fd);
        return AMVENC_FAIL;
    }
    ...
    if (!hw_info->dev_data)
    {
        VLOG(ERR,"InitAMVEncode Fail, dev type:%d. fd:%d", hw_info->dev_id, fd);
        hw_info->dev_id = NO_DEFINE;
        close(fd);
        return AMVENC_FAIL;
    }
