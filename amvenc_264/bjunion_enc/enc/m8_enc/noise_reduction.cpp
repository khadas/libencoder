...
int noise_reduction_init(PRM_NR *pPrm_Nr, int nImgW, int nImgH) {
    char prop[PROPERTY_VALUE_MAX];
    int value = 0;
    memset(prop, 0, sizeof(prop));
#ifdef SUPPORT_STANDARD_PROP
    if (property_get("vendor.hw.encoder.nr.enable", prop, NULL) < 0) {
#else
    ...
    pPrm_Nr->prm_bld_yalp = 128; // blend coefficient alpha for Y, higher with lower QP, 0~255
    pPrm_Nr->prm_bld_calp = pPrm_Nr->prm_bld_yalp; // blend coefficient alpha for UV, higher with lower QP,, 0~255
#ifdef SUPPORT_STANDARD_PROP
    if (property_get("vendor.hw.encoder.nr.alp", prop, NULL) > 0) {
#else
...
int fast_gau_filter(int *pGauOut,   // output
        ...
        int nCol, int nRow, int nWinSz,  // 0:3x3, 1:5x5
        int nIntMode,  // 0:normal, 1: interlaced (for NV21)
        int nImgW, int nImgH, int mode) {
    int i, j, k;
    ...
    for (nRow = 0; nRow < nImgH; nRow++) {
        for (nCol = 0; nCol < nImgW; nCol++) {
            ...
            if (prm_snr_ymode == 0) {
                ...
                pNrY[nRow * nImgW + nCol] = prm_snr_en == 0 ? pCurY[nRow * nImgW + nCol] : (prm_bld_yalp * nGau[0] + (256 - prm_bld_yalp) * pCurY[nRow * nImgW + nCol] + 128) / 256;
            }
            if (prm_snr_cmode == 0) {
                ...
                if (prm_is_nv21 == 0) { // 420p format
                    if (nRow % 2 == 0 && nCol % 2 == 0) {
                        ...
                        pNrU[nRow / 2 * nImgW / 2 + nCol / 2] = prm_snr_en == 0 ? pCurU[nRow / 2 * nImgW / 2 + nCol / 2] : (prm_bld_calp * nGau[1] + (256 - prm_bld_calp) * pCurU[nRow / 2 * nImgW / 2 + nCol / 2] + 128) / 256;

                        fast_gau_filter(&nGau[2], pCurV, pVAccV, &nHAccV, nCol / 2, nRow / 2, prm_cwin, 0, nImgW / 2, nImgH / 2, pPrm_Nr->mode);
                        pNrV[nRow / 2 * nImgW / 2 + nCol / 2] = prm_snr_en == 0 ? pCurV[nRow / 2 * nImgW / 2 + nCol / 2] : (prm_bld_calp * nGau[2] + (256 - prm_bld_calp) * pCurV[nRow / 2 * nImgW / 2 + nCol / 2] + 128) / 256;
                    }
                } else { // For NV21 input (UV in one buffer), call gaussian_filter(...) only once:
                    if (nRow % 2 == 0) {
                        fast_gau_filter(&nGau[1], pCurUV, pVAccUV, nHAccUV, nCol, nRow / 2, prm_cwin, 1, nImgW, nImgH / 2, pPrm_Nr->mode);
                        pNrUV[nRow / 2 * nImgW + nCol] = prm_snr_en == 0 ? pCurUV[nRow / 2 * nImgW + nCol] : (prm_bld_calp * nGau[1] + (256 - prm_bld_calp) * pCurUV[nRow / 2 * nImgW + nCol] + 128) / 256;
                    }
                }
            }
        } //j
