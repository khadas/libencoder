...
AMVEnc_Status M8RCInitFrameQP(void *dev, void *rc, bool IDR, int bitrate, float frame_rate) {
    ...
    if (rateCtrl->rcEnable == true) {
        ...
        if (rateCtrl->encoded_frames == 0) {
            ...
            ALOGV("rateCtrl->bits_per_second, %d,rateCtrl->estimate_fps:%f, rateCtrl->frame_position:%d, rateCtrl->target:%d, rateCtrl->bitsPerFrame:%d", rateCtrl->bits_per_second, rateCtrl->estimate_fps, rateCtrl->frame_position, rateCtrl->target,
...
void* M8InitRateControlModule(amvenc_initpara_t* init_para) {
    ...
    CLEANUP_RC: M8CleanupRateControlModule((void*) rateCtrl);
    return NULL;
}
