...
static int set_input(gx_fast_enc_drv_t* p, ulong *yuv, uint32_t enc_width, uint32_t enc_height, AMVEncBufferType type, AMVEncFrameFmt fmt) {
    ...
    if (AMVENC_NV21 == fmt || AMVENC_NV12 == fmt || AMVENC_YUV420 == fmt) {
        src->framesize = pitch * height * 3 / 2;
        VLOG(NONE,"framesize:%d",src->framesize);
    }
    ...
    if ((type == PHYSICAL_BUFF) && (pitch % 32 != 0)) {
        if ((src->plane[0] != 0) && (src->mmapsize != 0)) {
            VLOG(DEBUG,"Release old mmap buffer :0x%lx, size:0x%lx. fd:%d", src->plane[0], src->mmapsize, p->fd);
            munmap((void*) src->plane[0], src->mmapsize);
            src->plane[0] = 0;
            src->mmapsize = 0;
        } else if ((src->plane[0] != 0) || (src->mmapsize != 0)) {
            VLOG(ERR,"Buffer Ptr error:0x%lx, size:0x%lx. fd:%d", src->plane[0], src->mmapsize, p->fd);
        }
        ...
        y = (ulong) mmap(0, src->mmapsize, PROT_READ | PROT_WRITE, MAP_SHARED, p->fd, (ulong) src->canvas);
        src->plane[0] = y;
        if ((fmt == AMVENC_NV21) || (fmt == AMVENC_NV12) || (fmt == AMVENC_YUV420))
    ...
    if (src->type == VMALLOC_BUFFER) {
        VLOG(NONE,"src->type == VMALLOC_BUFFER");
...
static void Prepare_CBR_BitsTable(gx_fast_enc_drv_t* p, bool rc)
{
    ...
    if (p->logtime && rc)
        VLOG(DEBUG,"Prepare_CBR_BitsTable: want:%d, real:%d, diff:%d", size, total_bits, size - total_bits);
}
...
static void smooth_tbl_mode(gx_fast_enc_drv_t* p, uint32_t tbl[]) {
  uint8_t* max_value;
  int qp_min, qp_max;
  float bitrate = p->bitrate;
  if (p == NULL)
    return;

  float pp = bitrate / p->enc_width / p->enc_height;
  //ALOGD("enter smooth_tbl_mode,pp:%f",pp);
  if (pp < 1) {
        if (p->IDRframe) {
        qp_min = 10;//10;
        qp_max = 38;//28;
...
//    }
    ...
    for (i = 0; i < 16; i++) {
        memcpy(tbl_addr, &tbl[0], sizeof(tbl));
        memcpy(tbl_addr+32, &tbl[0], sizeof(tbl));
...
//    }
    ...
    if (media_custom > 0) {
        pp = p->bitrate / p->enc_width / p->enc_height;
        //ALOGD("PP = %f \n",pp);
        if (pp > 2.6) {
            //ALOGD("change to use Fill_CBR_Table");
            Fill_CBR_Table(p, (p->make_qptl == ADJUSTED_QP_FLAG) ? true : false);
        }
		else {
            //ALOGE("use Fill_CBR_Table_Custom");
            Fill_CBR_Table_Custom(p, (p->make_qptl == ADJUSTED_QP_FLAG) ? true : false);
        }
    } else {
        //ALOGE("use Fill_CBR_Table");
        Fill_CBR_Table(p, (p->make_qptl == ADJUSTED_QP_FLAG) ? true : false);
    }
...
//        media_value = 1;
   media_value = 0;
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"start_ime_cbr: poll fail, fd:%d", p->fd);
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            VLOG(NONE,"start_ime_cbr: done size: %d, fd:%d", result[0], p->fd);
        }
    } else {
        VLOG(ERR,"start_ime_cbr: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_PICTURE_READY) {
        ...
        if (p->logtime) {
            ...
            VLOG(DEBUG,"start_ime_cbr: need time: %d us, base qp: %d, frame num:%d, fd:%d", total_time, p->quant, p->total_encode_frame, p->fd);
...
static AMVEnc_Status start_intra_cbr_twice(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    AMVEnc_Status ret = AMVENC_FAIL;
    uint32_t status;
    uint32_t i;
    uint32_t result[4];
...
//    }
    ...
    control_info[5] = p->src.framesize;
    control_info[6] = p->quant;
    control_info[7] = AMVENC_FLUSH_FLAG_INPUT | AMVENC_FLUSH_FLAG_DUMP | AMVENC_FLUSH_FLAG_CBR;
    info_off = 17 + tbl_offset * 3;
    ...
    if (media_custom > 0) {
        ...
        pp = p->bitrate / p->enc_width / p->enc_height;
        if (pp > 2.6) {
            //ALOGE("change to use Fill_CBR_Table");
            Fill_CBR_Table(p, (p->make_qptl == ADJUSTED_QP_FLAG) ? true : false);
        }
        else {
            //ALOGE("use Fill_CBR_Table_Custom");
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"start_intra_cbr_twice: poll fail, fd:%d", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            VLOG(NONE,"start_intra_cbr_twice: done size: %d, fd:%d", result[0], p->fd);
        }
    } else {
        VLOG(ERR,"start_intra_cbr_twice: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            p->total_encode_time += total_time;
            VLOG(DEBUG,"start_intra_cbr_twice: need time: %d us, base qp: %d, frame num:%d, fd:%d", total_time, p->quant, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}

static AMVEnc_Status start_intra_cbr(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    AMVEnc_Status ret = AMVENC_FAIL;
    uint32_t status;
    uint32_t i;
    uint32_t result[4];
...
//    }
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"start_intra_cbr: poll fail, fd:%d", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            VLOG(NONE,"start_intra_cbr: done size: %d, fd:%d", result[0], p->fd);
        }
    } else {
        VLOG(ERR,"start_intra_cbr: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            p->total_encode_time += total_time;
            VLOG(DEBUG,"start_intra_cbr: need time: %d us, base qp: %d, frame num:%d, fd:%d", total_time, p->quant, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}

static AMVEnc_Status start_ime_one_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    AMVEnc_Status ret = AMVENC_FAIL;
    uint32_t status;
    uint32_t i;
    uint32_t result[4];
    ...
    control_info[5] = p->src.framesize;
    control_info[6] = p->quant;
    control_info[7] = AMVENC_FLUSH_FLAG_INPUT | AMVENC_FLUSH_FLAG_DUMP | AMVENC_FLUSH_FLAG_OUTPUT;
    info_off = 17 + tbl_offset * 3;
    if (p->make_qptl == ADJUSTED_QP_FLAG) {
        control_info[6] = ADJUSTED_QP_FLAG;
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"start_ime_one_pass: poll fail, fd:%d", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_PICTURE_READY;
            VLOG(NONE,"start_ime_one_pass: done size: %d, fd:%d", result[0], p->fd);
        }
    } else {
        VLOG(ERR,"start_ime_one_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_PICTURE_READY) {
        ...
        if (p->logtime) {
            ...
            total_time = p->end_test.tv_sec - p->start_test.tv_sec;
            total_time = total_time * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            VLOG(DEBUG,"start_ime_one_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
            p->total_encode_time += total_time;
        }
    }
    return ret;
...
static AMVEnc_Status start_intra_one_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    AMVEnc_Status ret = AMVENC_FAIL;
    uint32_t status;
    uint32_t i;
    uint32_t result[4];
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"start_intra_one_pass: poll fail, fd:%d", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            ...
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            VLOG(NONE,"start_intra_one_pass: done size: %d, fd:%d", result[0], p->fd);
        }
    } else {
        VLOG(ERR,"start_intra_one_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            p->total_encode_time += total_time;
            VLOG(DEBUG,"start_intra_one_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}

static AMVEnc_Status start_ime_two_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    AMVEnc_Status ret = AMVENC_FAIL;
    uint32_t status;
    uint32_t i;
    uint32_t result[4];
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"start_ime: poll fail, fd:%d", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_PICTURE_READY;
            VLOG(NONE,"start_ime_two_pass: done size: %d, fd:%d", result[0], p->fd);
        }
    } else {
        VLOG(ERR,"start_ime_two_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if (encode_poll(p->fd, -1) <= 0) {
            VLOG(ERR,"start_ime_two_pass: poll fail, fd:%d", p->fd);
            return AMVENC_TIMEOUT;
        }
    ...
    if (ret == AMVENC_PICTURE_READY) {
        ...
        if (p->logtime) {
            ...
            total_time = p->end_test.tv_sec - p->start_test.tv_sec;
            total_time = total_time * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            VLOG(DEBUG,"start_ime_two_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
            p->total_encode_time += total_time;
        }
    }
    return ret;
...
static AMVEnc_Status start_intra_two_pass(gx_fast_enc_drv_t* p, unsigned char* outptr, int* datalen) {
    AMVEnc_Status ret = AMVENC_FAIL;
    uint32_t status;
    uint32_t i;
    uint32_t result[4];
    ...
    control_info[14] = p->src.crop.src_h;
    control_info[15] = p->scale_enable ? 1 : 0;
    VLOG(DEBUG,"ucode_mode:%d",p->ucode_mode);
    if (p->scale_enable)
        p->src.fmt = AMVENC_NV21;
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"start_intra_two_pass: poll fail, fd:%d", p->fd);
        return AMVENC_TIMEOUT;
    }
    ...
    if (status == ENCODER_IDR_DONE) {
        ...
        if ((result[0] < p->output_buf.size) && (result[0] > 0)) {
            *datalen = result[0];
            ...
            Parser_DumpInfo(p);
            ret = AMVENC_NEW_IDR;
            VLOG(NONE,"start_intra_two_pass: done size: %d, fd:%d", result[0], p->fd);
        }
    } else {
        VLOG(ERR,"start_intra_two_pass: encode timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    ...
    if (ret == AMVENC_NEW_IDR) {
        ...
        if (p->logtime) {
            ...
            total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
            p->total_encode_time += total_time;
            VLOG(DEBUG,"start_intra_two_pass: need time: %d us, frame num:%d, fd:%d", total_time, p->total_encode_frame, p->fd);
        }
    }
    return ret;
}

static int get_block_resolution(gx_fast_enc_drv_t* p, uint32_t mb_width, uint32_t mb_height)
{
    ...
    VLOG(INFO,"EB block width:%d, height:%d mb_width:%d, mb_height:%d, block_width_num:%d, block_height_num:%d, block_width_n:%d, block_height_n:%d, block_width:%d, block_height:%d",
...
void* GxInitFastEncode(int fd, amvenc_initpara_t* init_para) {
    ...
    uint32_t mode = init_para->ucode_mode;
    gx_fast_enc_drv_t* p = NULL;
    int i = 0;
    double pp;
    if (!init_para) {
        VLOG(ERR,"InitFastEncode init para error.  fd:%d", fd);
        return NULL;
    }

    p = (gx_fast_enc_drv_t*) calloc(1, sizeof(gx_fast_enc_drv_t));
    if (!p) {
        VLOG(ERR,"InitFastEncode calloc faill. fd:%d", fd);
        return NULL;
    }
    ...
    p->fd = fd;
    if (p->fd < 0) {
        VLOG(ERR,"InitFastEncode open encode device fail, fd:%d", p->fd);
        free(p);
        return NULL;
    }
    ...
    if ((ret) || (buff_info[0] == 0)) {
        VLOG(ERR,"InitFastEncode -- old venc driver. no buffer information! fd:%d", p->fd);
        free(p);
        return NULL;
    }
    ...
    if (p->mmap_buff.addr == MAP_FAILED) {
        VLOG(ERR,"InitFastEncode mmap fail, fd:%d", p->fd);
        free(p);
        return NULL;
    }
...
//    }
    ...
    if (pp < 3) {
        ...
        if (ret)
            VLOG(DEBUG,"set qp mode failed!\n");
        else
            VLOG(DEBUG,"set qp mode successfully!");
        }
    }
    ...
    p->ucode_mode = init_para->ucode_mode;
	p->bitrate = init_para->bitrate;
    VLOG(DEBUG,"p->ucode_mode:%d",p->ucode_mode);
    p->sps_len = 0;
    p->gotSPS = false;
    p->pps_len = 0;
    p->gotPPS = false;
    ...
    buff_info[4] = init_para->color_space;
    ret = ioctl(p->fd, FASTGX_AVC_IOC_CONFIG_INIT,&buff_info[0]);
    if (ret) {
        VLOG(ERR,"InitFastEncode config init fai, fd:%dl", p->fd);
    ...
    if (p->mb_info == NULL) {
        VLOG(ERR,"ALLOC mb info memory failed. fd:%d", p->fd);
        munmap(p->mmap_buff.addr, p->mmap_buff.size);
        free(p);
        return NULL;
    }
...
//        }
        if (value & 0x8) {
            VLOG(DEBUG,"Enable Debug Time Log, fd:%d", p->fd);
            p->logtime = true;
        }
...
AMVEnc_Status GxFastEncodeInitFrame(void *dev, ulong *yuv, AMVEncBufferType type, AMVEncFrameFmt fmt, bool IDRframe) {
    ...
    if (p->logtime) {
        ...
        total_time = (p->end_test.tv_sec - p->start_test.tv_sec) * 1000000 + p->end_test.tv_usec - p->start_test.tv_usec;
        p->total_encode_time += total_time;
        VLOG(DEBUG,"GxVEncodeInitFrame: need time: %d us, ret:%d, fd:%d", total_time, ret, p->fd);
    }
    return ret;
}
...
AMVEnc_Status GxFastEncodeSPS_PPS(void* dev, unsigned char* outptr, int* datalen) {
    ...
    if (encode_poll(p->fd, -1) <= 0) {
        VLOG(ERR,"sps pps: poll fail, fd:%d", p->fd);
        return AMVENC_TIMEOUT;
    }

    ioctl(p->fd, FASTGX_AVC_IOC_GET_STAGE, &status);

    VLOG(DEBUG,"FastEncodeSPS_PPS status:%d, fd:%d,ucode_mode:%d", status, p->fd,p->ucode_mode);
    ret = AMVENC_FAIL;
    if (status == ENCODER_PICTURE_DONE) {
        ...
        VLOG(ERR,"sps pps timeout, status:%d, fd:%d", status, p->fd);
        ret = AMVENC_TIMEOUT;
    }
    if (outptr && *datalen >= 6) {
        if (outptr[5] == 0x4d) {
            VLOG(DEBUG,"Encode in CABAC mode");
            p->cabac_mode = true;
        }
    }
    return ret;
...
AMVEnc_Status GxFastEncodeSlice(void* dev, unsigned char* outptr, int* datalen) {
    gx_fast_enc_drv_t* p = (gx_fast_enc_drv_t*) dev;
    AMVEnc_Status ret = AMVENC_FAIL;
    if ((!p) || (!outptr) || (!datalen))
        return ret;
...
void GxUnInitFastEncode(void* dev) {
    ...
    if (p->logtime)
        VLOG(DEBUG,"total_encode_frame: %d, total_encode_time: %d ms, fd:%d", p->total_encode_frame, p->total_encode_time / 1000, p->fd);
    free(p);
    return;
}
