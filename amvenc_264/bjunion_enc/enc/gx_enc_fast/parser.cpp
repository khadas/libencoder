...
void adjust_qp_table(gx_fast_enc_drv_t* p)
{
    ...
    for (int i = 0; i < QP_TAB_NUM; i++) {
        if (p->qp_stic.p16_count[i] != 0) {
            ...
            if (p->re_encode == false) {
                ...
                VLOG(NONE,"p16 i:%d : aj:%lf avr_bits:%d count:%d lstqs:%lf",
                        i,
                        q_step_aj,
                ...
                VLOG(NONE,"p16 i:%d avr_bits:\t%d \tcount:\t%d", i, p->qp_stic.p16_avr_bits[i], p->qp_stic.p16_count[i]);
            }
        } else {
        ...
        if (p->qp_stic.i4_count[i] != 0) {
            ...
            if (p->re_encode == false) {
                ...
                VLOG(NONE,"i4 i:%d aj:%lf avr_bits:%d count:%d lstqs:%lf",
                        i,
                        q_step_aj,
                ...
                VLOG(NONE,"i4 i:%d avr_bits:\t%d \tcount:\t%d", i, p->qp_stic.i4_avr_bits[i], p->qp_stic.i4_count[i]);
            }
        } else {
        ...
        if (p->qp_stic.i16_count[i] != 0) {
            ...
            if (p->re_encode == false) {
                ...
                VLOG(NONE,"i16 i:%d aj:%lf avr_bits:%d count:%d lstqs:%lf",
                        i,
                        q_step_aj,
                ...
                VLOG(NONE,"i16 i:%d :avr_bits:\t%d \tcount:\t%d", i, p->qp_stic.i16_avr_bits[i], p->qp_stic.i16_count[i]);
            }
        } else {
    ...
    VLOG(INFO,"i16:%s\n", str[1]);
    VLOG(INFO,"p16:%s\n", str[2]);
    VLOG(INFO,"bits_per_mb:%d", bits_per_mb);
#endif
}
...
//    if (property_get("vendor.hw.encoder.mb.dump", prop, "3") > 0) {
//#else
//    if (property_get("hw.encoder.mb.dump", prop, "3") > 0) {
//#endif
//        sscanf(prop, "%d", &value);
//    }
    if (value > 20 || value < 0) {
        VLOG(ERR,"error hw.encoder.mb.dump: value: %d", value);
    }
    frame_count++;
#endif
    ...
    for (y = 0; y < p->src.mb_height; y++) {
        ...
        for (x = 0; x < p->src.mb_width; x++) {
            ...
            if (((uint32_t)x != info->mbx) || ((uint32_t)y != info->mby)) {
                VLOG(ERR,"parser mb poistion error: actual: %dx%d, info:%dx%d", x, y, info->mbx, info->mby);
                return -1;
            }
            info->mb_type = get_mb_type(cur_mb);
            if ((info->mb_type == HENC_MB_Type_I4MB) || (info->mb_type == HENC_MB_Type_I16MB)) {
                ...
                if (info->inter.sad < info->intra.sad) {
                    get_mb_mv_P16x16(cur_mb, info->inter.mv);
                    info->final_sad = info->inter.sad - p->me_weight;
                    VLOG(NONE,"frame:%d, parser mb (%dx%d) type %d warning--set as inter 16x16, cur_mb:0x%x, inter sad:%d, intra sad:%d",
                    ...
                    info->intra.LPred[0] = get_mb_LPred_I16(cur_mb);
                    info->final_sad = info->intra.sad - p->i16_weight;
                    VLOG(NONE,"frame:%d, parser mb (%dx%d) type %d warning--set as I16(mode %d), cur_mb:0x%x, inter sad:%d, intra sad:%d",
                            p->total_encode_frame + 1,
                            x,
                            y,
                            info->mb_type,
                ...
                VLOG(ERR,"parser mb (%dx%d) type %d error, cur_mb:0x%x", x, y, info->mb_type, (ulong) (cur_mb - p->dump_buf.addr));
                return -1;
            }
            if (info->final_sad < 0)
                info->final_sad = 0;
            ...
            if (info->bits > 4 * p->target / p->src.mb_width / p->src.mb_height) {
                VLOG(INFO,"frame_count%d y:%d x:%d", frame_count, y, x);
                char str[10];
                memset(str, 0, sizeof(str));
                strcat(str_line, "t:");
...
#ifdef SAD_INTERVAL_STATISTIC
    VLOG(INFO,"p mb: count:\t%d \t0:\t%d \t128:\t%d \t256:\t%d \t512:\t%d \t1024:\t%d \t2048:\t%d",
            count, count_0, count_128, count_256, count_512, count_1024, count_2048);
    VLOG(INFO,"i4 mb: count:\t%d \t0:\t%d \t128:\t%d \t256:\t%d \t512:\t%d \t1024:\t%d \t2048:\t%d",
            i_4_count, i_4_count_0, i_4_count_128, i_4_count_256, i_4_count_512, i_4_count_1024, i_4_count_2048);
    VLOG(INFO,"i16 mb: count:\t%d \t0:\t%d \t128:\t%d \t256:\t%d \t512:\t%d \t1024:\t%d \t2048:\t%d",
