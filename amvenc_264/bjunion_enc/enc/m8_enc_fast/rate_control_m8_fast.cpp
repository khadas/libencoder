...
void* FastInitRateControlModule(amvenc_initpara_t* init_para) {
    ...
    if (property_get("ro.vendor.amlenc.maxqp.I", qp_str, NULL) > 0) {
#else
    ...
    memset(qp_str, 0, sizeof(qp_str));
#ifdef SUPPORT_STANDARD_PROP
    if (property_get("ro.vendor.amlenc.maxqp.P", qp_str, NULL) > 0) {
#else
...
#ifdef SUPPORT_STANDARD_PROP
    if (property_get("ro.vendor.amlenc.qpstep.MaxInc", qp_str, NULL) > 0) {
#else
...
#ifdef SUPPORT_STANDARD_PROP
    if (property_get("ro.vendor.amlenc.qpstep.MaxDec", qp_str, NULL) > 0) {
#else
