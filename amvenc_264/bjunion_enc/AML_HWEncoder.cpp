...
AMVEnc_Status DetermineFrameNum(AMVEncHandle *Handle, amvenc_info_t* info, uint32 modTime, uint32 new_frame_rate, bool force_IDR) {
    ...
    if (info->first_frame) {
        ...
        currFrameNum = (int32) (((modTime - modTimeRef) * (float) info->frame_rate + border) / 1000); /* add small rounding */
        ...
        if (info->frame_in_gop >= (uint) info->idrPeriod && info->idrPeriod > 0) { /* first frame or IDR*/
            ...
            if (force_IDR) {  // todo: force_IDR only need one IDR or reset gop.
                VLOG(NONE,"force next frame to idr :%d, handle: %p.", force_IDR, Handle);
                info->nal_unit_type = AVC_NALTYPE_IDR;
                info->slice_type = AVC_I_SLICE;
            } else {
    ...
    VLOG(NONE,"Get Nal Type: %s,  handle: %p.", (info->nal_unit_type == AVC_NALTYPE_IDR) ? "IDR" : "SLICE", Handle);
    return AMVENC_SUCCESS;
}
...
AMVEnc_Status AML_HWEncInitialize(AMVEncHandle *Handle, AMVEncParams *encParam, bool* has_mix, int force_mode) {
    ...
    info->ucode_mode = encParam->ucode_mode;
    ...
    info->hw_info.init_para.color_space = encParam->color_space;

    //add for ucode mode
    info->hw_info.init_para.ucode_mode = info->ucode_mode;
    VLOG(DEBUG,"info->hw_info.init_para.color_space=%#x  ,ucode_mode:%d", info->hw_info.init_para.color_space,info->hw_info.init_para.ucode_mode);
    ...
    *has_mix = (info->hw_info.dev_id == M8) ? true : false;
    VLOG(DEBUG,"AML_HWEncInitialize success, handle: %p, fd:%d", Handle, info->hw_info.dev_fd);
    return AMVENC_SUCCESS;
exit:
    if (info) {
    ...
    VLOG(ERR,"AML_HWEncInitialize Fail, error=%d. handle: %p  ", status, Handle);
    return status;
}
...
AMVEnc_Status AML_HWSetInput(AMVEncHandle *Handle, AMVEncFrameIO *input) {
    ...
    if (info == NULL) {
        VLOG(ERR,"AML_HWSetInput Fail: UNINITIALIZED. handle: %p", Handle);
    ...
    if (info->state == AMVEnc_WaitingForBuffer) {
        ...
        VLOG(ERR,"AML_HWSetInput Wrong state: %d. handle: %p", info->state, Handle);
        return AMVENC_FAIL;
    }
    if (input->pitch > 0xFFFF) {
        VLOG(ERR,"AML_HWSetInput Fail: NOT_SUPPORTED. handle: %p", Handle);
    ...
    if (AMVENC_SUCCESS != DetermineFrameNum(Handle, info, input->coding_timestamp, (uint32) input->frame_rate, (input->op_flag & AMVEncFrameIO_FORCE_IDR_FLAG))) {
        VLOG(DEBUG,"AML_HWSetInput SKIPPED_PICTURE, handle: %p", Handle);
    ...
    VLOG(NONE,"AML_HWSetInput status: %d. handle: %p", status, Handle);
    return status;
}
...
AMVEnc_Status AML_HWGetAvgQp(AMVEncHandle *Handle, float *avgqp) {
    ...
    if (info == NULL) {
        VLOG(ERR,"AML_HWEncNAL Fail: UNINITIALIZED. handle: %p", Handle);
        return AMVENC_UNINITIALIZED;
    }
...
AMVEnc_Status AML_HWEncNAL(AMVEncHandle *Handle, unsigned char *buffer, unsigned int *buf_nal_size, int *nal_type) {
    ...
    VLOG(NONE,"AML_HWEncNAL state: %d. handle: %p", info->state, Handle);
    switch (info->state) {
        ...
        case AMVEnc_Encoding_SPS:
            ...
            if (ret != AMVENC_SUCCESS) {
                VLOG(NONE,"AML_HWEncNAL state: %d, err=%d, handle: %p", info->state, ret, Handle);
                return ret;
            }
            if (info->outOfBandParamSet)
                info->state = AMVEnc_Analyzing_Frame;
        ...
        case AMVEnc_Encoding_Frame:
            if (*nal_type == 0xff) {
                VLOG(DEBUG,"AML_HWEncNAL Cancel Current Frame. handle: %p", Handle);
                info->state = AMVEnc_Analyzing_Frame;
                ret = AMVENC_SUCCESS;
                return ret;
            }
            ...
            if (ret == AMVENC_TIMEOUT) {
                VLOG(DEBUG,"AML_HWEncNAL state: %d, err=%d. handle: %p", info->state, ret, Handle);
            ...
            *buf_nal_size = datalen;
            ...
            if ((ret == AMVENC_PICTURE_READY) || (ret == AMVENC_NEW_IDR)) {
                ...
                if (ret == AMVENC_REENCODE_PICTURE) {
                    ret = AMVEncodeSlice(&info->hw_info, buffer, &datalen);
                    if ((ret != AMVENC_PICTURE_READY) && (ret != AMVENC_SUCCESS) && (ret != AMVENC_NEW_IDR)) {
                        VLOG(ERR,"AML_HWEncNAL state: %d, err=%d. handle: %p", info->state, ret, Handle);
                        return ret;
                    }
                    info->nal_unit_type = (ret == AMVENC_NEW_IDR) ? AVC_NALTYPE_IDR : info->nal_unit_type;
                    *nal_type = info->nal_unit_type;
                    *buf_nal_size = datalen;
                    if (ret == AMVENC_NEW_IDR) {
                        ...
                        VLOG(NONE,"re-encode failed:%d. handle: %p", ret, Handle);
                    }
                }
                if (AMVEncodeCommit(&info->hw_info, (info->nal_unit_type == AVC_NALTYPE_IDR)) != AMVENC_SUCCESS) {
                    VLOG(ERR,"Encode Commit  failed:%d. handle: %p", ret, Handle);
                } else {
                    info->frame_in_gop++;
                }
                info->state = AMVEnc_Analyzing_Frame;
   ...
   VLOG(NONE,"AML_HWEncNAL next state: %d, ret=%d. handle: %p", info->state, ret, Handle);
    return ret;
}
...
AMVEnc_Status AML_HWEncRelease(AMVEncHandle *Handle) {
    ...
    Handle->object = NULL;
    VLOG(NONE,"AML_HWEncRelease Done, handle: %p", Handle);
    return AMVENC_SUCCESS;
}
