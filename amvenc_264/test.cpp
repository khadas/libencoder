...
bool get_valid_int_param(char *linestr,const char *var_str,int *param) {
    char *valid_str = NULL;
    char sub_str[256] = {0};
...
bool get_valid_str_param(char *linestr,const char *var_str,char *param) {
    char *valid_str = NULL;
    char sub_str[256] = {0};
    char end_str_len = 0;
    if (NULL == var_str || NULL == linestr || NULL == param) {
...
static void *encode_thread(void *param) {
    ...
    vl_frame_type_t frame_type = FRAME_TYPE_NONE;
    vl_enc_result_e result = ENC_FAILED;
    ...
    unsigned int output_size  = 1024 * 1024 * sizeof(char);
    ...
    if (pconfig_param->buf_type == DMA_BUFF)
    {
        ...
        printf("input[0] %p, input[1] %p,input[2] %p\n", input[0],
               input[1], input[2]);
        frame_info.plane_num = dma_info.num_planes;
        frame_info.YCbCr[0] = dma_info.shared_fd[0];
        frame_info.YCbCr[1] = dma_info.shared_fd[1];
        frame_info.YCbCr[2] = dma_info.shared_fd[2];
    } else {
        ...
        frame_info.YCbCr[0] = (ulong) (input_buffer);
        frame_info.YCbCr[1] =
            (ulong) (input_buffer + ysize);
        frame_info.YCbCr[2] =
            (ulong) (input_buffer + ysize + usize);
    }
    ...
    init_param.width = pconfig_param->width;
    init_param.height = pconfig_param->height;
    init_param.frame_rate = pconfig_param->framerate;
    init_param.bit_rate = pconfig_param->bitrate;
    init_param.gop = pconfig_param->gop;
    init_param.i_qp_min = pconfig_param->i_qp_min;
    init_param.i_qp_max = pconfig_param->i_qp_max;
    init_param.p_qp_min = pconfig_param->p_qp_min;
    init_param.p_qp_max = pconfig_param->p_qp_max;

    frame_info.type = (vl_buffer_type_e)pconfig_param->buf_type;
    frame_info.frame_type = FRAME_TYPE_AUTO;
    frame_info.frame_size = framesize;

    frame_info.fmt = (vl_img_format_t)pconfig_param->fmt;
    frame_info.pitch = pconfig_param->width;
    frame_info.height = pconfig_param->height;
    ...
    frame_info.bitrate = pconfig_param->bitrate;
    frame_info.frame_rate = pconfig_param->framerate * 1000;

    handle_enc = vl_video_encoder_init(CODEC_ID_H264, init_param);
    ...
    while (pconfig_param->num > 0) {
        ...
        result = vl_video_encoder_encode_frame(handle_enc, frame_info, output_buffer, &datalen, &frame_type);
...
void print_help() {
    ...
    printf("  fmt      : encode input fmt 1:nv12 2:nv21 3:yv12 4:rgb888 5:bgr888\n ");
...
int main(int argc, const char *argv[])
{
	...
	pthread_t tid[MAX_ENCODER_INSTANCE];
    int iloop = 0;
    if (argc < 2)
    {
        print_help();
