...
debug_log_level_t g_vp4_log_level = ERR;

void vp4_set_log_level(debug_log_level_t level)
{
    char *log_level = getenv("VP4_LOG_LEVEL");
    if (log_level) {
        g_vp4_log_level = (debug_log_level_t)atoi(log_level);
        printf("Set log level by environment to %d\n", g_vp4_log_level);
    } else {
        g_vp4_log_level = level;
